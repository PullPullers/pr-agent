...
def get_model(model_type: str = "model_weak") -> str:
    ...
    elif model_type == "model_reasoning" and get_settings().get(
...
def convert_to_markdown_v2(
    ...
    if get_settings().get("pr_reviewer.enable_intro_text", False):
        markdown_text += (
            f"Here are some key observations to aid the review process:\n\n"
        )
        ...
        if "Estimated effort to review" in key_nice:
            ...
            if gfm_supported:
                ...
                if is_value_no(value):
                    ...
                    markdown_text += (
                        f"{emoji}&nbsp;<strong>ToDo sections</strong><br><br>\n\n"
                    )
                    ...
                    for todo_item in value:
                        ...
                        if git_provider and file_name and line_number:
                            reference_link = git_provider.get_line_link(
                                file_name, line_number, line_number
                            )
                ...
                if is_value_no(value):
                    ...
                    if isinstance(value, list):
                        for todo_item in value:
                            ...
                            if content.strip():
                                markdown_text += (
                                    f"- {file_name} [{line_number}]: {content}\n"
                                )
                            else:
                                markdown_text += f"- {file_name} [{line_number}]\n"
                    else:
                        markdown_text += f"{value}\n\n"
            ...
            if is_value_no(value):
                ...
                if gfm_supported:
                    ...
                    markdown_text += (
                        f"### {emoji} Recommended focus areas for review\n\n#### \n"
                    )
                for i, issue in enumerate(issues):
                    try:
                        ...
                        if issue_header.lower() == "possible bug":
                            issue_header = (
                                "Possible Issue"  # Make the header less frightening
                            )
...
def ticket_markdown_logic(emoji, markdown_text, value, gfm_supported) -> str:
    ...
    if isinstance(value, list):
        ...
        if gfm_supported:
            markdown_text += f"<tr><td>\n\n"
            markdown_text += (
                f"**{emoji} Ticket compliance analysis {compliance_emoji}**\n\n"
            )
...
def try_fix_yaml(
    ...
    if not snippet:
        snippet = re.search(
            snippet_pattern, response_text_original
        )  # before we removed the "```"
    if snippet:
...
def get_max_tokens(model):
    ...
    if model in MAX_TOKENS:
        ...
        get_logger().error(
            f"Model {model} is not defined in MAX_TOKENS in ./pr_agent/algo/__init__.py and no custom_model_max_tokens is set"
        )
        raise Exception(
