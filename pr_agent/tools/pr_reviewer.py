...
class PRReviewer:
    ...
    def _prepare_pr_review(self) -> str:
        ...
        review_todos: list[str] = []
        for f in self.git_provider.get_diff_files():
            if f.edit_type == EDIT_TYPE.DELETED:
                continue
            todos = get_todo_sections(f.patch, f.filename)
            if todos:
                review_todos.extend(todos)

        if review_todos:
            data["review"]["Todo sections"] = review_todos
