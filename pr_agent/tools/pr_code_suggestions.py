...
from .pr_description import insert_br_after_x_chars
...
class PRCodeSuggestions:
    def __init__(
        ...
        if get_settings().pr_code_suggestions.get("decouple_hunks", True):
            ...
            self.pr_code_suggestions_prompt_system = (
                get_settings().pr_code_suggestions_prompt_not_decoupled.system
            )
            self.pr_code_suggestions_prompt_user = (
                get_settings().pr_code_suggestions_prompt_not_decoupled.user
            )
    ...
    async def run(self):
        try:
            ...
            data = await retry_with_fallback_models(
                self.prepare_prediction_main, model_type=ModelType.REGULAR
            )
            if not data:
    ...
    async def publish_no_suggestions(self):
        ...
        if (
            get_settings().config.publish_output
            and get_settings().pr_code_suggestions.get(
    ...
    async def _get_prediction(
        ...
        if (
            model_reflect_with_reasoning == get_settings().config.model
            and model != get_settings().config.model
            and fallbacks
            and model == fallbacks[0]
        ):
            # we are using a fallback model (should not happen on regular conditions)
            get_logger().warning(
                f"Using the same model for self-reflection as the one used for suggestions"
            )
            model_reflect_with_reasoning = model
        response_reflect = await self.self_reflect_on_suggestions(
            data["code_suggestions"], patches_diff, model=model_reflect_with_reasoning
        )
        if response_reflect:
    ...
    async def analyze_self_reflection_response(self, data, response_reflect):
        ...
        if code_suggestions_feedback and len(code_suggestions_feedback) == len(
            ...
            for i, suggestion in enumerate(data["code_suggestions"]):
                ...
                suggestion = self.validate_one_liner_suggestion_not_repeating_code(
                    suggestion
                )
        ...
        try:
            ...
            for file in diff_files:
                if file.filename.strip() == relevant_file:
                    ...
                    if (
                        existing_code in base_file
                        and existing_code not in head_file
                        and new_code in head_file
                    ):
                        suggestion["score"] = 0
                        get_logger().warning(
            ...
            get_logger().exception(
                f"Error validating one-liner suggestion", artifact={"error": e}
            )
    ...
    async def convert_to_decoupled_with_line_numbers(
        ...
        with get_logger().contextualize(
            sub_feature="convert_to_decoupled_with_line_numbers"
        ):
            try:
                patches_diff_list = []
                for patch_prompt in patches_diff_list_no_line_numbers:
                    ...
                    if token_count > max_tokens_full - delta_output:
                        ...
                        patch_final = clip_tokens(
                            patch_final, max_tokens_full - delta_output
                        )
    ...
    def generate_summarized_suggestions(self, data: Dict) -> str:
        try:
            ...
            for label, suggestions in suggestions_labels.items():
                ...
                pr_body += (
                    f"""<tr><td rowspan={num_suggestions}>{label.capitalize()}</td>\n"""
                )
    ...
    async def self_reflect_on_suggestions(
        ...
        try:
            ...
            with get_logger().contextualize(command="self_reflect_on_suggestions"):
                response_reflect, finish_reason_reflect = (
